...
def start_keyboard(message):
    ...
    bot.send_message(message.chat.id, 'Хуба-буба', reply_markup=murkup)

def paymenu_keyboard(message):
    murkup = types.ReplyKeyboardMarkup(True, False)
    murkup.row('игра', 'Кошелёк')
    murkup.row('Назад')
    bot.send_message(message.chat.id, 'Файв', reply_markup=murkup)
...
def about_keyboard(message):
    ...
    murkup.row('Назад')
    bot.send_message(message.chat.id, 'Орбит', reply_markup=murkup)
...
def paygame_keyboard(message):
    ...
    murkup.row('Назад')
    bot.send_message(message.chat.id, 'Малабар', reply_markup=murkup)

def freegame_keyboard(message):
    murkup = types.ReplyKeyboardMarkup(True, False)
    murkup.row('Поиск игры')
    murkup.row('Назад')
    bot.send_message(message.chat.id, 'Хер', reply_markup=murkup)
...
def wallet_keyboard(message):
    ...
    murkup.row('Назад')
    bot.send_message(message.chat.id, 'Пенис', reply_markup=murkup)
